...
void signal_callback_handler(int signum) {
	...
	//now for closing the proccess:
	if (!CreateProcessA("C:\\Users\\test0\\OneDrive\\מסמכים\\MagshimimSimesterB\\Gallery\\gallary_yonatan_cohen_2023\\galleryProject\\testing.exe", NULL, NULL, NULL, FALSE, 0, NULL, NULL, (LPSTARTUPINFOA)&si, &pi))
	{
		std::cout << "Didnt work to terminate the process" << std::endl;
	}
...
AlbumManager::AlbumManager(IDataAccess& dataAccess) :
    m_dataAccess(dataAccess), m_nextPictureId(105), m_nextUserId(200)
{
	// Left empty
	m_dataAccess.open();
}
...
void AlbumManager::showPicture()
{
	...
	//creating the cmd line..
	//if you want to use ifranview you just need to write this :
	//std::string ifranView = "C:\Program Files\IrfanView\i_view64.exe " + pic.getPath();
	...
	std::string before = pic.getPath();
	for (int i = 0; i < pic.getPath().length(); i++)
	{
		if (before[i] == '\\')
		{
			fixed_path += before[i];
	...
	std::string ifranView = "C:\\Windows\\system32\\mspaint.exe \"" + pic.getPath() + "\"";
	if (!CreateProcessA(NULL, const_cast<LPSTR>(ifranView.c_str()), NULL, NULL, FALSE, 0, NULL, NULL, (LPSTARTUPINFOA)&si1, &pi1))
	{
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "albums of the user : " << std::endl;
	std::list<Album> list = m_dataAccess.getAlbumsOfUser(user);
